...
from script_path import ANNOTATIONS_PATH, ANNOTATIONS_OUTPUT, LABELS_PATH, LABELS_OUTPUT, jpath
...
def filter(split):
    ...
    with open(jpath(ANNOTATIONS_PATH, f"instances_{split}2017.json"), 'r') as f:
        data = json.load(f)
    ...
    with open(jpath(ANNOTATIONS_OUTPUT, f"{split}_person.txt"), 'w') as f:
        for img_id in sorted(person_image_ids):
            f.write(f"./images/{split}2017/" + id_to_filename[img_id] + '\n')
    text = jpath(ANNOTATIONS_OUTPUT, f"{split}_person.txt")
    print(f"Список сохранен в {text}")

    if os.path.exists(jpath(LABELS_PATH, f"{split}2017")):
        print("2. Фильтрую YOLO-лейблы...")
        os.makedirs(jpath(LABELS_OUTPUT, f"{split}2017"), exist_ok=True)

        person_filenames = {Path(name).stem for name in id_to_filename.values()}

        copied = 0
        for label_file in os.listdir(jpath(LABELS_PATH, f"{split}2017")):
            if label_file.endswith('.txt'):
                stem = Path(label_file).stem
                if stem in person_filenames:
                    src = jpath(LABELS_PATH, f"{split}2017", label_file)
                    dst = jpath(LABELS_OUTPUT, f"{split}2017", label_file)
                    filter_labels_in_file(src, dst)
                    copied += 1
        text = jpath(LABELS_OUTPUT, f"{split}2017")
...
def main():
    filter('train')
    with open(jpath(ANNOTATIONS_OUTPUT, f"train_person.txt"), 'r') as f:
    ...
    with open(jpath(ANNOTATIONS_OUTPUT, f"train_person_split.txt"), 'w') as f:
        f.write('\n'.join(train_images))

    with open(jpath(ANNOTATIONS_OUTPUT, f"test_person.txt"), 'w') as f:
        f.write('\n'.join(test_images))
